...
public class UserService {
     ...
     * This method has a 50% chance of throwing a RuntimeException to simulate a service failure.
     * @return A list of user IDs.
     */
    public List<String> getAllUserIds() {
        if (ThreadLocalRandom.current().nextInt(10) < 5) {
            throw new RuntimeException("External User Service is unavailable");
        }
    ...
    public List<String> getUserIdsByRole(String role) {
         if (ThreadLocalRandom.current().nextInt(10) < 5) {
            throw new RuntimeException("External User Service is unavailable for role: " + role);
        }
