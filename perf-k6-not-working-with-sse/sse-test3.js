...
export const options = {
  scenarios: {
    ...
    listeners: {
      ...
      startVUs: 0,
      stages: [
        { duration: '20s', target: 10 }, // Ramp up 10 listeners
        { duration: '40s', target: 10 }, // They stay connected and listen
      ],
      exec: 'listen', // This scenario will execute the 'listen' function
      gracefulRampDown: '10s',
    },
    // Scenario for the single user who will create the broadcast
    broadcaster: {
      executor: 'per-vu-iterations',
      vus: 1,
      iterations: 1, // Run only once
      startTime: '63s', // Start after the listeners have had time to connect
...
export function listen() {
  // Use the VU ID to pick a user from the array
  ...
  const response = sse.open(sseUrl, {}, function (client) {
    ...
    // VUs will wait up to 50 seconds to receive a message before timing out.
    // This should be longer than the test stage duration.
    sleep(50);
    client.close();
  });
